...
public class PlayerStatController : MonoBehaviour
{
    ...
    public void UpdateDisplay()
    {
        ...
        {
            UIController.instance.moveSpeedUpgradeDispalay.UpdateDispaly(moveSpeed[moveSpeedLevel + 1].cost, moveSpeed[moveSpeedLevel].value, moveSpeed[moveSpeedLevel + 1].value);
        } else
        {
            UIController.instance.moveSpeedUpgradeDispalay.ShowMaxLevel();
        }

        if(healthLevel < health.Count - 1)
        {
            UIController.instance.healthUpgradeDisplay.UpdateDispaly(health[healthLevel + 1].cost, health[healthLevel].value, health[healthLevel + 1].value);
        }else
        {
        ...
        {
            UIController.instance.pickupRangeUpgradeDisplay.UpdateDispaly(pickupRange[pickupRangeLevel + 1].cost, pickupRange[pickupRangeLevel].value, pickupRange[pickupRangeLevel + 1].value);
        }else
        {
        ...
        {
            UIController.instance.maxWeaponsUpgradeDisplay.UpdateDispaly(maxWeapons[maxWeaponsLevel + 1].cost, maxWeapons[maxWeaponsLevel].value, maxWeapons[maxWeaponsLevel + 1].value);
        } else
        {
